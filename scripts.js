class PortfolioGenerator {
    ...
    validateInputs(github, linkedin) {
        ...
        if (!linkedin) {
            this.showError('linkedin-error', 'LinkedIn URL is required');
            isValid = false;
        } else if (!linkedin.match(/^https:\/\/[w]{0,3}\.?linkedin\.com\/.*$/)) {
    ...
    updatePreview(userData, repos, linkedin) {
        ...
        previewFrame.addEventListener('load', () => {
            // Force the iframe to execute scripts
            const iframeDocument = previewFrame.contentDocument || previewFrame.contentWindow.document;
            
            // Manually initialize the theme switcher
            const themeSwitch = iframeDocument.querySelector('.theme-switch');
            ...
            // Manually initialize the projects display
            const projectsContainer = iframeDocument.getElementById('projects');
            const username = this.githubInput.value.trim();
            
            try {
                // Get the cached repos data (already sorted by stars)
                ...
                const seeAllLink = iframeDocument.createElement('div');
...
<head>
    ...
    <link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2'><circle cx='12' cy='12' r='10'/><line x1='2' y1='12' x2='22' y2='12'/><path d='M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z'/></svg>">
    <style>
...
async function fetchGitHubProjects() {
    ...
    async generateFiles() {
        ...
        // Add CSS file
        zip.file('style.css', this.getPortfolioStyles());
